...
export const addLink = (link: string, name: string) => (dispatch: Dispatch): void => {
  const data: IData = {
    newLink: link,
    name: name,
...
export const deleteLink = (link: string) => (dispatch: Dispatch): void => {
  const data: IData = {
    token: localStorage.getItem('token'),
    linkToRemove: link,
  };
...
export const loadLinks = () => (dispatch: Dispatch) => {
  const data: IData = {
    token: localStorage.getItem('token'),
  };
  fetch('/loadlinks', {
